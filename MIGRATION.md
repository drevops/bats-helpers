...
| `mock_set_output`, `mock_set_status`, `mock_set_side_effect`, `mock_spec_set_output`, `mock_spec_set_status`, `mock_spec_set_side_effect` | The value was written with `echo -e`, so a value of `-n`, `-e` or `-E` was read as a flag and swallowed, and a backslash escape such as `\n` was expanded | The value is written literally and reaches the caller unchanged. Write a multi-line response with ANSI-C quoting: `mock_set_output "${mock}" $'AccessDenied\n403'` |
| `mock_create`, `mock_command` | A call that a mock's expectations did not cover was answered with the default status and output | Mocks are strict by default, and such a call fails with `Mock 'git' received a call that no expectation covers: git 'log'`. Declare the uncovered calls, or set `BATS_HELPERS_MOCK_STRICT=0` before the first mock is created |
| `steps_run` | A `@` step carrying a newline was truncated at it, silently dropping the status, the output and the side effect | Such a step is rejected. Keep it on one line, continuing a long one with a trailing `\`, and write a newline in its output as `\n`, which the output field expands |
