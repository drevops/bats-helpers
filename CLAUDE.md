...
- **State the actual fact, and append an expectation clause only when the fact alone does not convey it.** A positive assertion states the negated fact with no clause (`Directory '${dir}' is not empty`); a negative assertion states the positive fact plus `, but should not` after a plain verb or `, but should not be` after a copula (`Directory '${dir}' is empty, but should not be`). Where a positive assertion can only state a positive fact, the clause carries the expectation (`Regular file '${file}' exists, but should be a symlink`).
